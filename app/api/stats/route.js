import { db } from '@/lib/firebase';
import { doc, getDoc, setDoc } from 'firebase/firestore';
import { TEAMS } from '@/lib/teams';

// Calculate player stats from a single match
function processMatch(match, teamId) {
  const isHome = match.homeTeam?.id === teamId;
  const team = isHome ? match.homeTeam : match.awayTeam;
  const players = {};

  const competition = match.competition?.code || 'UNKNOWN';
  const matchInfo = {
    id: match.id,
    date: match.utcDate,
    opponent: isHome ? match.awayTeam?.shortName : match.homeTeam?.shortName,
    homeAway: isHome ? 'H' : 'A',
    score: `${match.score?.fullTime?.home}-${match.score?.fullTime?.away}`,
    competition,
  };

  // Process starting lineup
  for (const p of team.lineup || []) {
    players[p.id] = {
      id: p.id,
      name: p.name,
      position: p.position,
      shirtNumber: p.shirtNumber,
      starts: 1,
      subApps: 0,
      minutesPlayed: 90,
      goals: 0,
      assists: 0,
      yellowCards: 0,
      redCards: 0,
      matches: [{ ...matchInfo, started: true, minutesPlayed: 90 }],
    };
  }

  // Process bench
  for (const p of team.bench || []) {
    players[p.id] = {
      id: p.id,
      name: p.name,
      position: p.position,
      shirtNumber: p.shirtNumber,
      starts: 0,
      subApps: 0,
      minutesPlayed: 0,
      goals: 0,
      assists: 0,
      yellowCards: 0,
      redCards: 0,
      matches: [],
    };
  }

  // Process substitutions — adjust minutes
  for (const sub of match.substitutions || []) {
    if (sub.team?.id !== teamId) continue;
    const minute = sub.minute || 90;

    // Player going off — reduce their minutes
    if (players[sub.playerOut?.id]) {
      players[sub.playerOut.id].minutesPlayed = minute;
      if (players[sub.playerOut.id].matches.length > 0) {
        players[sub.playerOut.id].matches[players[sub.playerOut.id].matches.length - 1].minutesPlayed = minute;
      }
    }

    // Player coming on — add sub appearance
    if (players[sub.playerIn?.id]) {
      players[sub.playerIn.id].subApps = 1;
      players[sub.playerIn.id].minutesPlayed = 90 - minute;
      players[sub.playerIn.id].matches = [{
        ...matchInfo,
        started: false,
        minutesPlayed: 90 - minute,
        cameOnMinute: minute,
      }];
    }
  }

  // Process goals
  for (const goal of match.goals || []) {
    if (goal.team?.id !== teamId) continue;
    if (players[goal.scorer?.id]) {
      players[goal.scorer.id].goals += 1;
    }
    if (goal.assist && players[goal.assist?.id]) {
      players[goal.assist.id].assists += 1;
    }
  }

  // Process bookings
  for (const booking of match.bookings || []) {
    if (booking.team?.id !== teamId) continue;
    if (players[booking.player?.id]) {
      if (booking.card === 'YELLOW') players[booking.player.id].yellowCards += 1;
      if (booking.card === 'RED' || booking.card === 'YELLOW_RED') players[booking.player.id].redCards += 1;
    }
  }

  return Object.values(players).filter(p => p.starts > 0 || p.subApps > 0);
}

export async function GET(request) {
  const { searchParams } = new URL(request.url);
  const teamId = parseInt(searchParams.get('teamId'));
  const competition = searchParams.get('competition') || 'all';

  if (!teamId) return Response.json({ error: 'teamId required' }, { status: 400 });

  const team = TEAMS.find(t => t.id === teamId);
  if (!team) return Response.json({ error: 'Team not found' }, { status: 404 });

  try {
    // Check Firestore cache
    const cacheKey = `stats_${teamId}_2026`;
    const cached = await getDoc(doc(db, 'player_stats', cacheKey));

    let playerStats = {};

    if (cached.exists()) {
      playerStats = cached.data().stats;
    } else {
      // Fetch all matches for this team's competition
      const res = await fetch(
        `https://api.football-data.org/v4/competitions/${team.competition}/matches?season=2026&status=FINISHED`,
        { headers: { 'X-Auth-Token': process.env.FOOTBALL_DATA_API_KEY } }
      );

      if (!res.ok) return Response.json({ error: 'Failed to fetch matches' }, { status: 500 });

      const data = await res.json();
      const teamMatches = (data.matches || []).filter(m =>
        m.homeTeam?.id === teamId || m.awayTeam?.id === teamId
      );

      // Fetch full detail for each match
      for (const match of teamMatches) {
        await new Promise(r => setTimeout(r, 100)); // small delay to avoid rate limits
        const matchRes = await fetch(
          `https://api.football-data.org/v4/matches/${match.id}`,
          {
            headers: {
              'X-Auth-Token': process.env.FOOTBALL_DATA_API_KEY,
              'X-Unfold-Lineups': 'true',
              'X-Unfold-Goals': 'true',
              'X-Unfold-Bookings': 'true',
              'X-Unfold-Subs': 'true',
            },
          }
        );
        if (!matchRes.ok) continue;
        const fullMatch = await matchRes.json();
        const matchPlayers = processMatch(fullMatch, teamId);

        // Merge into playerStats
        for (const p of matchPlayers) {
          if (!playerStats[p.id]) {
            playerStats[p.id] = {
              id: p.id,
              name: p.name,
              position: p.position,
              shirtNumber: p.shirtNumber,
              starts: 0,
              subApps: 0,
              minutesPlayed: 0,
              goals: 0,
              assists: 0,
              yellowCards: 0,
              redCards: 0,
              matches: [],
            };
          }
          playerStats[p.id].starts += p.starts;
          playerStats[p.id].subApps += p.subApps;
          playerStats[p.id].minutesPlayed += p.minutesPlayed;
          playerStats[p.id].goals += p.goals;
          playerStats[p.id].assists += p.assists;
          playerStats[p.id].yellowCards += p.yellowCards;
          playerStats[p.id].redCards += p.redCards;
          playerStats[p.id].matches.push(...p.matches);
        }
      }

      // Cache in Firestore
      await setDoc(doc(db, 'player_stats', cacheKey), {
        stats: playerStats,
        teamId,
        updatedAt: new Date().toISOString(),
      });
    }

    // Convert to array and filter by competition if needed
    let players = Object.values(playerStats);

    if (competition !== 'all') {
      players = players.map(p => ({
        ...p,
        matches: p.matches.filter(m => m.competition === competition),
        starts: p.matches.filter(m => m.competition === competition && m.started).length,
        subApps: p.matches.filter(m => m.competition === competition && !m.started).length,
        minutesPlayed: p.matches.filter(m => m.competition === competition).reduce((s, m) => s + m.minutesPlayed, 0),
        goals: 0, // recalculated below
        assists: 0,
        yellowCards: 0,
        redCards: 0,
      }));
    }

    // Sort by appearances then name
    players.sort((a, b) =>
      (b.starts + b.subApps) - (a.starts + a.subApps) ||
      a.name.localeCompare(b.name)
    );

    return Response.json({ players, teamId, competition });
  } catch (err) {
    return Response.json({ error: err.message }, { status: 500 });
  }
}